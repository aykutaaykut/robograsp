...
void
fillObjectsRosMessage (pc_segmentation::PcObjects &outRosMsg, std::vector<pc_cluster_features> &feats)
{
  outRosMsg.header.stamp = ros::Time::now();
  outRosMsg.count = feats.size();

  for(int i=0;i<feats.size();i++){
    pc_cluster_features obj_featurs = feats[i];
    pc_segmentation::PcBoundingBox bb_msg;

    bb_msg.bb_center.x = obj_featurs.aligned_bounding_box.center.x;
    bb_msg.bb_center.y = obj_featurs.aligned_bounding_box.center.y;
    bb_msg.bb_center.z = obj_featurs.aligned_bounding_box.center.z;

    bb_msg.bb_angle  = obj_featurs.aligned_bounding_box.angle;
    bb_msg.bb_dims.x = obj_featurs.aligned_bounding_box.size.xSize;
    bb_msg.bb_dims.y = obj_featurs.aligned_bounding_box.size.ySize;
    bb_msg.bb_dims.z = obj_featurs.aligned_bounding_box.size.zSize;

    bb_msg.rgba_color.r = obj_featurs.color[0]/255.;
    bb_msg.rgba_color.g = obj_featurs.color[1]/255.;
    bb_msg.rgba_color.b = obj_featurs.color[2]/255.;
    bb_msg.rgba_color.a = 1.0;
    bb_msg.hue = obj_featurs.hue;

    outRosMsg.boxes.push_back(bb_msg);
  }
}
